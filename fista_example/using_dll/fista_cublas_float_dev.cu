...
extern "C"
__global__ void soft_thr_S(float *x, float alpha, float *S, int length)
{
    float thr = 0;
...
extern "C"
__host__ void FISTA(float *beta, float *X, float *y, float *lambda, float *L, float *Eta, float *tolerance, float *Loss, int *max_iteration, int *N, int *P, int *steps)
{
    ...
    float One = 1.0, MOne = -1.0, Zero=0.0;
    ...
    unsigned int bpg_p = (int) ceil(((float) p)/TPB.x);
    BPG.x = bpg_p;
    ...
    float *d_One, *d_MOne, *d_Zero;
    float eta_ik, L_cur, pL_cur, RHS, LHS, tnext, t1;
    float h_rbp, h_RHS_1st, h_RHS_2nd, h_rb, h_crit;
    float *d_rbp, *d_RHS_1st, *d_RHS_2nd, *d_rb, *d_crit;
    float t = 1.0, *d_t1, *d_pL_cur;
    bool cond;
    ...
    for (k = 0; k < max_iter; k++)
    {
        ...
        cublasSgemv(handle, ntran, n, p, d_MOne, d_X, n, 
                    d_beta_p, 1, d_One, d_ymXbp, 1);
        cublasSdot(handle, n, d_ymXbp, 1, d_ymXbp, 1, d_rbp);
        cudaMemcpy(&h_rbp, d_rbp, sizeof(float), DTH);
        cublasSgemv(handle, tran, n, p, d_One, d_X, n, 
        ...
        while(cond)
        {
            ...
            L_cur = L_prev * eta_ik;
            pL_cur = 1.0 / L_cur;
            cudaMemcpy(d_pL_cur, &pL_cur, sizeof(float), HTD);
            cudaMemcpy(d_bstar, d_beta_p, sz_p, DTD);
            cublasSaxpy(handle, p, d_pL_cur, d_XTrbp, 1, d_bstar, 1);
            soft_thr_S<<<BPG, TPB>>>(d_bstar, lam/L_cur, d_beta, p);

            cudaMemcpy(d_diff_beta, d_beta, sz_p, DTD);
            cublasSaxpy(handle, p, d_MOne, d_beta_p, 1, d_diff_beta, 1);
            cublasSdot(handle, p, d_diff_beta, 1, d_diff_beta, 1, d_RHS_1st);
            cublasSdot(handle, p, d_diff_beta, 1, d_XTrbp, 1, d_RHS_2nd);
            ...
            cublasSdot(handle, n, d_ymXb, 1, d_ymXb, 1, d_rb);
        ...
        cudaMemcpy(d_diff_beta, d_beta, sz_p, DTD);
        cublasSaxpy(handle, p, d_MOne, d_beta_prev, 1, d_diff_beta, 1);
        t1 = (t - 1.0) / tnext;
        cudaMemcpy(d_t1, &t1, sz_1, HTD);
        cudaMemcpy(d_beta_p, d_beta, sz_p, DTD);
        cublasSaxpy(handle, p, d_t1, d_diff_beta, 1, d_beta_p, 1);
        cublasSdot(handle, p, d_diff_beta, 1, d_diff_beta, 1, d_crit);
